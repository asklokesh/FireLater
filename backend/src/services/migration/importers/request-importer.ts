/**
 * Request Importer
 * Imports service requests from parsed migration data
 */
...
export class RequestImporter {
  /**
   * Import requests into tenant schema
   */
  async import(
    ...
    try {
      ...
      for (let i = 0; i < records.length; i++) {
        try {
          ...
          const validation = fieldMapper.validateMappedData(mapped.targetData, 'request');
          if (!validation.valid) {
            errors.push({
              recordIndex: i,
          ...
          logger.error({ err: error, recordIndex: i }, 'Failed to import request');
          errors.push({
            recordIndex: i,
      ...
      logger.error({ err: error, jobId: options.jobId }, 'Request import failed');
      throw error;
    } finally {
      client.release();
    }
  ...
  private async findExistingRequest(
    ...
    const result = await client.query(
      `SELECT id FROM ${schema}.catalog_requests WHERE external_id = $1 LIMIT 1`,
      [externalId]
    );
  ...
  private async insertRequest(
    ...
    const optionalFields = [
      'description', 'external_id', 'requested_for_email',
      'assigned_to_email', 'assigned_group', 'category',
      'approval_status', 'fulfillment_notes',
    ];
    ...
    const query = `
      INSERT INTO ${schema}.catalog_requests (${fields.join(', ')})
  ...
  private async updateRequest(
    ...
    const updatableFields = [
      ...
      'requested_for_email', 'assigned_to_email', 'assigned_group',
      'category', 'approval_status', 'fulfillment_notes', 'closed_at',
    ];
    ...
    const query = `
      UPDATE ${schema}.catalog_requests
      SET ${updates.join(', ')}
      WHERE id = $${paramIndex}
    `;
  ...
  private async trackImportedRecord(
    ...
    await client.query(
      ...
      SET target_id = EXCLUDED.target_id,
          ...
          imported_at = NOW()`,
      [jobId, tenantId, 'request', sourceId, targetId, targetSchema, targetTable, JSON.stringify(recordData)]
    );
  }
    ...
    try {
      ...
      const records = await client.query(
        `SELECT target_id, target_schema, target_table
         ...
         WHERE migration_job_id = $1 AND entity_type = 'request'`,
        [jobId]
      );
      ...
      logger.info({ jobId, deletedCount }, 'Request rollback completed');

      return deletedCount;
    } catch (error) {
      await client.query('ROLLBACK');
      logger.error({ err: error, jobId }, 'Request rollback failed');
      throw error;
    } finally {
      client.release();
    }
...
export const requestImporter = new RequestImporter();
